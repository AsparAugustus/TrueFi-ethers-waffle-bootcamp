...
describe('Token Splitter', () => {
  ...
  beforeEach(async () => {
    ...
    splitter = await deployContract(alice, TokenSplitter__factory, [token.address, bob.address, charlie.address]);
  });
  ...
  it('Splits correctly', async () => {
    ...
    await token.transferFrom(alice.address, splitter.address, 100)
    expect(await token.balanceOf(charlie.address)).to.above(0)
  })
});
